...
class extension {
    ...
    /**
     * Wrapper providing modern namespace fallback without altering the original legacy discovery.
     * If legacy discovery yields results they are returned as-is; otherwise we attempt to resolve
     * a class path based on the subnamespace (portion after mod_bigbluebuttonbn in the base FQN).
     *
     * @param string $classname Fully qualified base class/interface name.
     * @return array sortorder => implementing class name
     */
    protected static function get_classes_implementing_with_fallback(string $classname): array {
        $legacy = self::get_classes_implementing($classname);
        if (!empty($legacy)) {
            return $legacy;
        }
        // Derive modern subnamespace (exclude class basename; portion after mod_bigbluebuttonbn).
        $classnamecomponents = explode('\\', $classname);
        $classbasename = array_pop($classnamecomponents);
        $modidx = array_search('mod_bigbluebuttonbn', $classnamecomponents);
        $subnamespaces = $modidx !== false ? array_slice($classnamecomponents, $modidx + 1) : [];
        $modernsubnamespace = $subnamespaces ? implode('\\', $subnamespaces) : '';
        if ($modernsubnamespace === '') { // No additional namespace to probe.
            return [];
        }
        $allsubs = core_plugin_manager::instance()->get_plugins_of_type(self::BBB_EXTENSION_PLUGIN_NAME);
        $extensionclasses = [];
        ...
        foreach ($allsubs as $sub) {
            ...
            $targetclassname = "\\bbbext_{$sub->name}\\$modernsubnamespace\\$classbasename";
            if (!class_exists($targetclassname)) {
                continue;
            }
    ...
    public static function get_rendered_output_override($renderer, $instance): ?string {
        $classes = self::get_classes_implementing_with_fallback(\mod_bigbluebuttonbn\output\view_page::class);
        if (!empty($classes)) {
